...
The shape of a single SKU manifest entry. Consumers either pass an array of these to `createIAP({ products: [...] })`, or have their backend return them via [`BackendAdapter.listProducts()`](/v5/api/backend-adapter#listproducts-optional) — see [Backend contract → `products`](/v5/guide/backend-contract#products-optional).
...
- [`createIAP`](/v5/api/create-iap) — the factory
- [Errors](/v5/api/errors) — `IAPError`, `IAPErrorCode`
- [Events reference](/v5/api/events-reference) — `EventMap` and friends
