...
The backend returns the SKU list during `initialize()` so you can feature-flag, A/B-test, or region-gate SKUs without an app release. Every id MUST still be pre-registered in App Store Connect / Google Play Console. Prefer a static array? Pass `products: [...]` — see [Configuration → products](/v5/guide/configuration#products).
:::
...
- [Vue + Quasar recipe](/v5/recipes/vue-quasar) — Pinia store wiring
- [Optimistic grant](/v5/recipes/optimistic-grant) — show premium UI before backend confirms
- [Events](/v5/guide/events) — what `entitlements-changed` and other events deliver
