...
Response shape and the App-Store-Connect-pre-registration caveat are documented in [Backend contract → `products`](/v5/guide/backend-contract#products-optional).
...
See [Backend contract](/v5/guide/backend-contract) for the request/response shapes the library expects.
...
When `adapter` is provided, the `baseUrl` / `endpoints` / `getAuthHeaders` fields become optional. See [`BackendAdapter`](/v5/api/backend-adapter) for the interface.
...
| `productPriceCacheTtlMs` | `86_400_000` (24h) | TTL for cached native product info (titles, prices). Currently informational — cached prices are not auto-refreshed on expiry. |
...
- [Backend contract](/v5/guide/backend-contract) — what your backend must implement
- [API: types](/v5/api/types) — full type definitions
