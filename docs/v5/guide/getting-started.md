...
- A backend you can deploy a few new endpoints to (or you can implement them as a [custom `BackendAdapter`](/v5/api/backend-adapter))
- An [Attesto](https://attesto.nossdev.com) tenant with API key

If you're missing any of these, the [Backend contract](/v5/guide/backend-contract) and [Testing on sandbox](/v5/guide/testing) pages cover them.
...
See [Vue + Quasar recipe](/v5/recipes/vue-quasar) and [React recipe](/v5/recipes/react) for full reactive store wiring.
...
- [Backend contract](/v5/guide/backend-contract) — implement the four endpoints in your backend (Attesto handles the receipt validation)
- [Configuration](/v5/guide/configuration) — full schema reference
- [Error handling](/v5/guide/error-handling) — all `IAPErrorCode` values and remediation hints
- [Testing on sandbox](/v5/guide/testing) — sandbox tester accounts on iOS + Google license testers on Android
