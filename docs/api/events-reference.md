...
For the canonical event ordering within a flow (purchase, restore, refresh), see [Events guide → ordering guarantees](/guide/events#ordering-guarantees).

## See also

- [Events guide](/guide/events) — full payload semantics with examples
- [`IAP.on`](/api/iap-instance#on-event-handler) — the `on` method signature
