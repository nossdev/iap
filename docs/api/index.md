...
| Page | Contents |
|---|---|
| [`createIAP`](/api/create-iap) | Factory function; entry point |
| [`IAP` instance](/api/iap-instance) | All instance methods |
| [Types](/api/types) | `Product`, `EntitlementBase`, `NativeTransaction`, `VerifiedTransaction`, `PurchaseResult`, `RestoreResult` |
| [Errors](/api/errors) | `IAPError`, `IAPErrorCode`, `errorHint`, `isIAPError` |
| [`BackendAdapter`](/api/backend-adapter) | Interface for custom transports |
| [Events reference](/api/events-reference) | `EventMap`, `EventName`, `EventPayload`, `Unsubscribe` |
