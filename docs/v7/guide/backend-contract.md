...
If your backend isn't HTTP/JSON (GraphQL, Firebase, Supabase, gRPC-web), implement a [`BackendAdapter`](/v7/api/backend-adapter) instead. The shapes below still apply at the domain level — only the wire encoding changes.
:::
...
Paths are configured via `config.backend.endpoints`. The defaults shown in [Configuration](/v7/guide/configuration) — `/api/iap/verify/apple` etc. — are convention, not required.
...
Runs on the parsed JSON before the library's zod validation. See [Configuration → backend](/v7/guide/configuration#backend).
...
- [Configuration](/v7/guide/configuration) — how to point the library at your endpoints
- [Error handling](/v7/guide/error-handling) — what your UI does with each error code
- [`BackendAdapter`](/v7/api/backend-adapter) — for non-HTTP transports
