...
- Acknowledges native transactions only **after** the backend confirms (see [Safety guarantees](/v7/guide/safety-guarantees)).
...
- Bridges to StoreKit 2 (iOS 15+) and Google Play Billing (Android API 21+).
...
See [Backend contract](/v7/guide/backend-contract) for the exact request/response shapes.
...
If anything fails between step 8 and step 11, the transaction stays in the unfinished queue — the next launch (or `iap.refresh()`) will retry from step 8. That's the [at-least-once recovery](/v7/guide/safety-guarantees#at-least-once-recovery) guarantee.
...
- [Safety guarantees](/v7/guide/safety-guarantees) — what `tx.finish()` after backend confirms means
- [Backend contract](/v7/guide/backend-contract) — the four endpoints your backend implements
- [Events](/v7/guide/events) — full event reference
