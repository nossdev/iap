...
On every `initialize()` (and optionally on app resume — see [`refreshOnResume`](/v7/guide/configuration#options)), the library:
...
See [Configuration: validation errors](/v7/guide/configuration#validation-errors).
...
There's no `iap.on('did-something', ...)` escape hatch — unknown event names are a compile error. See [Events](/v7/guide/events) for the full list.
...
- [Backend contract](/v7/guide/backend-contract) — exact request/response shapes
- [Error handling](/v7/guide/error-handling) — every `IAPErrorCode` and how to recover
- [Architecture](/v7/guide/architecture) — how the layers compose
