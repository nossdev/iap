...
For the purchase flow specifically, the discriminated union from `iap.purchase()` is more ergonomic than try/catch — see [Getting started → Buy something](/v7/guide/getting-started#_4-buy-something). Use try/catch for `restorePurchases()`, `refresh()`, and `initialize()`.
:::
...
| `BACKEND_BAD_RESPONSE` | no | 4xx / malformed JSON / schema mismatch | Backend doesn't match [contract](/v7/guide/backend-contract) |
...
- [Backend contract](/v7/guide/backend-contract) — what your backend returns on each path
- [`IAPError` API](/v7/api/errors) — full type reference
