...
Response shape and the App-Store-Connect-pre-registration caveat are documented in [Backend contract → `products`](/v7/guide/backend-contract#products-optional).
...
See [Backend contract](/v7/guide/backend-contract) for the request/response shapes the library expects.
...
When `adapter` is provided, the `baseUrl` / `endpoints` / `getAuthHeaders` fields become optional. See [`BackendAdapter`](/v7/api/backend-adapter) for the interface.
...
- [Backend contract](/v7/guide/backend-contract) — what your backend must implement
- [API: types](/v7/api/types) — full type definitions
