...
Type definitions for the event system. For a guide-style introduction with usage patterns, see [Events](/v7/guide/events).
...
For the canonical event ordering within a flow (purchase, restore, refresh), see [Events guide → ordering guarantees](/v7/guide/events#ordering-guarantees).

## See also

- [Events guide](/v7/guide/events) — full payload semantics with examples
- [`IAP.on`](/v7/api/iap-instance#on-event-handler) — the `on` method signature
