...
See [Configuration](/v7/guide/configuration) for the full schema.
...
See [`BackendAdapter`](/v7/api/backend-adapter) for the interface.
...
Configs that supply neither `products` nor a way for the backend to provide them throw `IAPError(INVALID_CONFIG)` at parse time. See [Backend contract → `products`](/v7/guide/backend-contract#products-optional) for the response shape and the App-Store-Connect-pre-registration caveat.
...
- [Configuration](/v7/guide/configuration) — every field
- [`IAP` instance](/v7/api/iap-instance) — methods on the returned object
- [Errors](/v7/api/errors) — `INVALID_CONFIG` shape
