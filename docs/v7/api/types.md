...
The shape of a single SKU manifest entry. Consumers either pass an array of these to `createIAP({ products: [...] })`, or have their backend return them via [`BackendAdapter.listProducts()`](/v7/api/backend-adapter#listproducts-optional) — see [Backend contract → `products`](/v7/guide/backend-contract#products-optional).
...
- [`createIAP`](/v7/api/create-iap) — the factory
- [Errors](/v7/api/errors) — `IAPError`, `IAPErrorCode`
- [Events reference](/v7/api/events-reference) — `EventMap` and friends
