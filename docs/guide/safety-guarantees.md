...
On every `initialize()` (and optionally on app resume — see [`refreshOnResume`](/guide/configuration#options)), the library:
...
See [Configuration: validation errors](/guide/configuration#validation-errors).
...
There's no `iap.on('did-something', ...)` escape hatch — unknown event names are a compile error. See [Events](/guide/events) for the full list.
...
- [Backend contract](/guide/backend-contract) — exact request/response shapes
